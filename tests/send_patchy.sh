#!/bin/bash
server_binary="../cpp-tftp-server-debug-build --drop 0.001 --duplicate 0.001";

if (( $# !=0 )); then
    echo "usage: bash send_netascii"
    exit 1
fi

num_simulated_connections=$1

echo "Running test send_patchy.sh."
echo "..."
...
if [[ -z $differences ]]; then
	...
	echo "..."
	echo "Test send_patchy.sh passed."
else
	echo $differences
	echo "..."
	echo "Differences were detected between the transferred file(s) and the original file."
	echo "Note: this test deliberately causes packet drop and duplication, so this"
	echo "probably isn't a bug."
	echo "..."
	echo "Test send_patchy.sh failed."
fi
